...
namespace DirectXHepler {
	...
	bool RootSignature::Initalize(ID3D12Device* device, const D3D12_ROOT_SIGNATURE_DESC& desc) {
		assert(device);
		...
		if (FAILED(D3D12SerializeRootSignature(
			...
			Logger::Error(static_cast<char*>(errorBlob->GetBufferPointer()));
			assert(false);
			return false;
		}
		...
		if (FAILED(device->CreateRootSignature(
			...
			Logger::Error("CreateRootSignature()");
			assert(false);
			return false;
		}
		return true;
